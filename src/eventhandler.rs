...
fn process_events(me: &mut dyn EventHandler, he: HELEMENT, evtg: UINT, params: LPVOID) -> BOOL {
	...
	let result = match evtg {
		EVENT_GROUPS::SUBSCRIPTIONS_REQUEST => {
			assert!(!params.is_null());
			let mut scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut EVENT_GROUPS) };
			let nm = unsafe { scnm.as_mut() };
		...
		EVENT_GROUPS::HANDLE_INITIALIZATION => {
			assert!(!params.is_null());
			let mut scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut INITIALIZATION_PARAMS) };
			let nm = unsafe { scnm.as_mut() };
			match nm.cmd {
		...
		EVENT_GROUPS::HANDLE_SOM => {
			assert!(!params.is_null());
			let mut scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut SOM_PARAMS) };
			let nm = unsafe { scnm.as_mut() };
			match nm.cmd {
		...
		EVENT_GROUPS::HANDLE_BEHAVIOR_EVENT => {
			assert!(!params.is_null());
			let scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut BEHAVIOR_EVENT_PARAMS) };
			let nm = unsafe { scnm.as_ref() };
		...
		EVENT_GROUPS::HANDLE_SCRIPTING_METHOD_CALL => {
			assert!(!params.is_null());
			let mut scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut SCRIPTING_METHOD_PARAMS) };
			let nm = unsafe { scnm.as_mut() };
		...
		EVENT_GROUPS::HANDLE_METHOD_CALL => {
			assert!(!params.is_null());
			let scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut METHOD_PARAMS) };
			let nm = unsafe { scnm.as_ref() };
		...
		EVENT_GROUPS::HANDLE_TIMER => {
			assert!(!params.is_null());
			let scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut TIMER_PARAMS) };
			let nm = unsafe { scnm.as_ref() };
		...
		EVENT_GROUPS::HANDLE_DRAW => {
			assert!(!params.is_null());
			let scnm = unsafe { std::ptr::NonNull::new_unchecked(params as *mut DRAW_PARAMS) };
			let nm = unsafe { scnm.as_ref() };
